    ...
    global.setupPersonLinkWidget = function setupPersonLinkWidget(options) {
        ...
        $field.closest('form').on('ajaxDialog:validateBeforeSubmit', function(evt) {
            ...
            if (req.primaryAuthor || req.secondaryAuthor || req.submitter || req.speaker) {
                ...
                if (req.submitter && _.indexOf(_.pluck(hiddenData, 'isSubmitter'), true) === -1) {
                    hasError = true;
                    $formField.data('error', $T.gettext('You must add at least one submitter'));
                }
                if (req.secondaryAuthor && _.indexOf(_.pluck(hiddenData, 'authorType'), 2) === -1) {
                    hasError = true;
                    $formField.data('error', $T.gettext('You must add at least one co-author'));
                }
                if (req.primaryAuthor && _.indexOf(_.pluck(hiddenData, 'authorType'), 1) === -1) {
                    hasError = true;
